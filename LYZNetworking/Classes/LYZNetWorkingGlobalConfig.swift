...
class LYZNetWorkingGlobalConfig: NSObject {
...
//                return "DELETE"
    ...
    enum LYZHTTPResponse {
       case data, json,xml
    }
    
    static let shared = LYZNetWorkingGlobalConfig()
    
    ///基础 url
    var apiDomain: String = ""
    ///超时时间
    var requestTimeOut: TimeInterval = 15 //默认 15 秒
    
    var requestSerialization: LYZHttpRequestSerialization = LYZHttpRequestSerialization()
    ...
    **NSURLRequestReloadRevalidatingCacheData = 5：**从原始地址确认缓存数据的合法性后，缓存数据就可以使用，否则从原始地址加载（在使用前去服务器验证）。**/
    ///全局缓存 策略
    var cachePolicy: NSURLRequest.CachePolicy = .useProtocolCachePolicy //默认的缓存策略
    
    ///会话配置
    var Configuration = URLSessionConfiguration.default  //默认配置 默认的URL会话配置对象，其存储方式是基于硬盘的持久化存储方式，会保存用户的证书到钥匙串中
    ///全局 header
    var headers: [String : String]?
    ///解析模式
    var responseSerializer: LYZHTTPResponse = .data
